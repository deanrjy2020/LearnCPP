...
// 3,
// malloc得到addr, 向后移动pointerSize, 得到ptr, 然后把ptr对齐, 得到userPtr
// 把addr的值放在userPtr的前面, 返回userPtr
// m是多少? 存addr要sizeof(void*)=8B大小存, 对齐最多要移动15B,
// 所以maxShift = sizeof(void*) + (align-1) = 8 + 15
// 所以要分配的内存大小memSize = userSize + maxShift;
// free的时候拿到ptr, 得到前面隐藏的addr值, 就是真正要free的内存.
...
public:
    void save(void* addr, void* userPtr) {
        this->addr = addr;
        this->userPtr = userPtr;
    }
    bool verify(void* addr, void* userPtr) {
        bool res = this->addr == addr && this->userPtr == userPtr;
...
void* aligned_malloc(size_t size, size_t align) {
    ...
    // 药分配的内存总大小
    size_t memSize = size + pointerSize + align - 1;
    ...
    uintptr_t ptr = (uintptr_t)addr + pointerSize;
    // 对齐, 有可能不会移动, 已过ptr已经是对齐的话.
    ...
    *(void**)preUserPtr = addr;

    // 测试, 保存起来
    dbg.save(addr, userPtr);
...
void aligned_free(void* ptr) {
    ...
    void* addr = *(void**)preUserPtr;

    // 测试
    assert(dbg.verify(addr, ptr));
